...
class AbstractLLMModel(ABC):
    ...
    @abstractmethod
    def chat_stream(self, messages, config={}) -> Iterator[str]:
        """
        Run the model with the given prompt and return the response.
        """
        pass
    ...
    @abstractmethod
    def generate(self, prompt, config={}) -> str:
        """
...
class OllamaModel(AbstractLLMModel):
    ...
    def chat(self, messages, config={}) -> str:
        response = ollama.chat(
            model=self.model_name.value, messages=messages, stream=False
        )
    ...
    def generate(self, prompt, config={}) -> str:
        response = ollama.generate(
            model=self.model_name.value, prompt=prompt, stream=False
        )
        return response.get("response", "")
...
class GeminiModel(AbstractLLMModel):
    ...
    def chat(self, messages, config={}) -> str:
        ...
        response = self.client.models.generate_content(
            model=self.model_name.value, contents=prompt, config=config
        )
